...
# NB: plain @pytest.fixture (anyio-managed), NOT @pytest_asyncio.fixture. The
# integration tests are @pytest.mark.anyio, so they run on anyio's per-test event
# loop. A pytest_asyncio.fixture sets the pool up on pytest-asyncio's loop instead,
# leaving the asyncpg pool bound to a DIFFERENT loop than the test — which surfaces
# under CI timing as "got Future attached to a different loop" / "another operation
# is in progress" / connection-closed cascades (it happened to coincide locally).
# Keeping the fixture anyio-managed guarantees pool and test share one loop.
@pytest.fixture
...
@pytest.fixture  # anyio-managed (see pg_pool note) — same-loop guarantee
