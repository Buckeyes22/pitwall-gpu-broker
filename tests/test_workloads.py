...
_SQL_TEST = _REPO_ROOT / "tests" / "sql" / "test_workloads.sql"
_MIGRATION_DIR = _REPO_ROOT / "db" / "migrations"
_TEST_POSTGRES_CONTAINER = "pitwall-test-postgres"


def test_workloads_state_check_and_idempotency_and_cost_precision() -> None:
    database_url = os.getenv("DATABASE_URL")
    if not database_url:
        pytest.skip("DATABASE_URL is required for the Postgres workloads test")

    result = _run_sql_test(database_url)

    assert result.returncode == 0, (
        f"workloads SQL test failed\nstdout:\n{result.stdout}\nstderr:\n{result.stderr}"
    )
