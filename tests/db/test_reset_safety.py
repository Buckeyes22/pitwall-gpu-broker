...
CREATE TABLE IF NOT EXISTS public._pwl_reset_test_sentinel (
    id   INT PRIMARY KEY,
    data TEXT NOT NULL
);
INSERT INTO public._pwl_reset_test_sentinel (id, data)
...
DECLARE
    v_data TEXT;
BEGIN
    SELECT data INTO v_data FROM public._pwl_reset_test_sentinel WHERE id = 1;
...
-- Cleanup
DROP TABLE IF EXISTS public._pwl_reset_test_sentinel;
...
CREATE TABLE neighbor._pwl_reset_test_sentinel (
    id   INT PRIMARY KEY,
    data TEXT NOT NULL
);
INSERT INTO neighbor._pwl_reset_test_sentinel (id, data)
...
DECLARE
    v_data TEXT;
BEGIN
    SELECT data INTO v_data FROM neighbor._pwl_reset_test_sentinel WHERE id = 1;
...
CREATE INDEX idx_pwl_test ON pitwall.capabilities (id);
