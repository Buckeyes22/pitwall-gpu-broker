...
_SQL_TEST = _REPO_ROOT / "tests" / "sql" / "test_provider_volume_secure_cloud.sql"
_MIGRATION_DIR = _REPO_ROOT / "db" / "migrations"
_TEST_POSTGRES_CONTAINER = "pitwall-test-postgres"


def test_volume_attached_provider_requires_secure_cloud() -> None:
    database_url = os.getenv("DATABASE_URL")
    if not database_url:
        pytest.skip("DATABASE_URL is required for the Postgres provider volume test")

    result = _run_sql_test(database_url)

    assert result.returncode == 0, (
        "provider volume secure-cloud SQL test failed\n"
        f"stdout:\n{result.stdout}\n"
        f"stderr:\n{result.stderr}"
    )
