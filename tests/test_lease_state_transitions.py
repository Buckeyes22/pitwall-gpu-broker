"""cover state storage - lease state transition tests."""
...
_SQL_TEST = _REPO_ROOT / "tests" / "sql" / "test_lease_state_transitions.sql"
_MIGRATION_DIR = _REPO_ROOT / "db" / "migrations"
_TEST_POSTGRES_CONTAINER = "pitwall-test-postgres"


def test_lease_state_transitions() -> None:
    database_url = os.getenv("DATABASE_URL")
    if not database_url:
        pytest.skip("DATABASE_URL is required for the Postgres lease state transition test")

    result = _run_sql_test(database_url)

    assert result.returncode == 0, (
        "lease state transition SQL test failed\n"
        f"stdout:\n{result.stdout}\n"
        f"stderr:\n{result.stderr}"
    )
...
def _sql_test_with_inlined_migrations() -> str:
    replacements = {
        ...
        r"\ir ../../db/migrations/0003_workloads.sql": (
            _MIGRATION_DIR / "0003_workloads.sql"
        ).read_text(),
        r"\ir ../../db/migrations/0004_leases.sql": (
            _MIGRATION_DIR / "0004_leases.sql"
        ).read_text(),
    }
    lines: list[str] = []
