...
_SQL_TEST = _REPO_ROOT / "tests" / "sql" / "test_lease_active_readiness.sql"
_MIGRATION_DIR = _REPO_ROOT / "db" / "migrations"
_TEST_POSTGRES_CONTAINER = "pitwall-test-postgres"


def test_lease_active_readiness_signals_check() -> None:
    database_url = os.getenv("DATABASE_URL")
    if not database_url:
        pytest.skip("DATABASE_URL is required for the Postgres lease readiness test")

    result = _run_sql_test(database_url)

    assert result.returncode == 0, (
        "lease active readiness SQL test failed\n"
        f"stdout:\n{result.stdout}\n"
        f"stderr:\n{result.stderr}"
    )
