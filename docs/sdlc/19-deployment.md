...
4. uploads and installs the exact version through TestPyPI before production publication;
...
Registry namespaces, trusted publishers, environment protection, branch rules, and the actual
publication rehearsal are external gates. Their required evidence is listed in
`docs/release/external-release-gates.md` and must not be inferred from a successful local build.
