...
Before setting the versioned `PITWALL_RELEASE_ENABLED` repository variable, the
project owner must create the repository and configure the protected
`ghcr-staging`, `ghcr`, and `github-release` environments and GHCR package
permissions. Python registry jobs additionally require
`PITWALL_PYPI_RELEASE_ENABLED=true`. That Python-channel variable must remain
unset until the PyPI/TestPyPI accounts, projects, rehearsal channel, trusted
publishers, and protected environments are ready.
