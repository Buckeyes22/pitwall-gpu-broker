...
| GOV-004, GOV-005 | Added governance, maintainer, support, security, CODEOWNERS, DCO, issue-template, and hosting-control documentation. Workflow policy tests pass. Private Vulnerability Reporting, secret scanning, push protection, Dependabot alerts/security updates, Discussions, signed web commits, immutable action references, and protected release environments were enabled and API-verified on GitHub on 2026-07-18. | **External gate:** branch rules must be enforced and the owner must add a backup maintainer or explicitly accept the documented single-maintainer risk. |
