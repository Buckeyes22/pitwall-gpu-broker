...
| GOV-004, GOV-005 | Added governance, maintainer, support, security, CODEOWNERS, DCO, issue-template, and hosting-control documentation. Workflow policy tests pass. | **External gate:** named backup maintainers, repository rules/security settings, monitored contacts, and Discussions must be configured and tested on GitHub. |
...
| TST-005 | **Verified.** Executable documentation/user journeys, internal-link validation, artifact smoke, image smoke, and clean-machine contracts are implemented. | Canonical source links cannot resolve until the reviewed initial source push; Discussions links also require that feature to be enabled. |
...
| DOC-001 through DOC-004 | **Verified.** Alpha claims, install/deploy/security/API/operator/privacy/support documentation, terminology scrub, internal links, and external-link automation are updated. | Canonical GitHub links remain blocked only by the reviewed initial source push and feature configuration. |
...
- Documentation: all 70 repository-local Markdown files pass link/anchor checks.
  Remaining canonical GitHub link resolution requires the reviewed initial source
  push and Discussions feature configuration.
